  ...
  return (
    <AppShell>
      <div className="space-y-8">
        ...
        <section className={`${insightHeroClass} px-5 py-6 sm:px-7 sm:py-7`} style={insightHeroStyle}>
          <div className="max-w-3xl">
            <span className={insightBadgeClass}>Scenarios available to you</span>
            <h2 className="mt-4 text-2xl font-semibold tracking-tight text-white sm:text-[2.2rem]">
              Choose a scenario and start practicing.
            </h2>
            <p className="mt-3 max-w-2xl text-[14px] leading-relaxed text-white/70">
              Repeated practice helps develop muscle memory.
            </p>
          </div>

          <div className="mt-6">
          {loading ? (
            <div className={`${insightGlassCardClass} py-8 text-center`}>
              <p className="text-[13px] text-white/70">Loading scenarios...</p>
            </div>
          ) : publishedScenarios.length === 0 ? (
            <div className={`${insightGlassCardClass} py-10 text-center`}>
              <p className="text-[13px] text-white/70">No scenarios available yet</p>
            </div>
          ) : (
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
              {publishedScenarios.map((s) => (
                <Link
                  ...
                  className={`group ${insightGlassCardClass} flex min-h-[168px] flex-col justify-between p-4 transition-colors hover:border-white/20 hover:bg-white/[0.08]`}
                >
                  <div>
                    <div className="mb-3 flex items-start justify-between gap-3">
                      <span className="rounded-full border border-white/10 bg-white/[0.08] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-white/60">
                        {s.difficulty}
                      </span>
                      <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/[0.08] text-white transition-colors group-hover:border-[#f0bb7e] group-hover:bg-[#f8a757] group-hover:text-slate-950">
                        <Play className="h-3.5 w-3.5" />
                      </div>
                    </div>
                    <p className="text-[15px] font-semibold leading-snug text-white">{s.title}</p>
                    {s.setting && (
                      <p className="mt-2 text-[12px] leading-snug text-white/60">{s.setting}</p>
                    )}
                  </div>

                  <div className="mt-4 flex items-center justify-end gap-3">
                    <span className="text-[12px] font-medium text-[#dcecff] transition-colors group-hover:text-white">
                      Start scenario
                    </span>
                  </div>
                </Link>
          ...
          <section>
            ...
            <div className="divide-y divide-slate-200 rounded-2xl border border-slate-200 bg-white shadow-sm">
              {sessions.map((s) => {
                ...
                return (
                  <div key={s.id} className="flex items-center justify-between px-4 py-2.5 transition-colors hover:bg-white/55">
